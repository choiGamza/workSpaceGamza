   ...
   <div id="wrapper">
      ...
      <ul class="sidebar navbar-nav"
         ...
         <li class="nav-item"
            onmouseover="this.style.backgroundColor='#333e48'"
            onmouseout="this.style.backgroundColor='#3a4651'"><a
            class="nav-link" href="userOrganization" style="background-color: #85ce36; color: #ffffff;"> <i
...
.img-circle {
	border-radius: 50%
        ...
        <c:if test = "${memberList.profileimg=='false' }">
        	<img class="rounded-circle img-fluid d-block mx-auto" src="http://placehold.it/150x150">
        </c:if>
        <c:if test = "${memberList.profileimg == 'true' }">
                   <img class="rounded-circle img-fluid d-block mx-auto" src="${pageContext.request.contextPath}/resources/profileimg/${memberList.id }.jpg" style = "width:150px; height : 150px;">
        </c:if>
          <h3>${memberList.name }
            <small>${memberList.position }</small>
          </h3>
          <small>${memberList.department }</small>
        </div>
        </c:forEach>
    </div>	 
    ...
    <div class="container3">
     	<ul>
			<c:forEach items = "${departmentList }" var = "departmentList">
				<c:if test = "${departmentList.department != '관리자' }">
				<li>
					<a href = "teamDetail?department=${departmentList.department}"> ${departmentList.department }</a>
				</li>
				</c:if>
