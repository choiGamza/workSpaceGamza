            ...
            <div class="collapse" id="collapse1">
               <ul class="nav navbar-nav">
                  <li class="nav-item"
                     ...
                     style="padding-left: 20px; background-color: #85ce36; color: #ffffff;"> <i class="far fa-fw fa-building"></i>
...
<form action = "userInfoUpdate" method = "post">
 <table class="table" style="text-align: left; border: 2px;">
 	<caption>${DetailUser.name }님의 상세정보</caption>
 	<tr>
 		<td colspan = "2"> user ID : ${DetailUser.id }
 		<input type = "hidden" name = "id" value = "${DetailUser.id }">
 		</td>
 	</tr>
 	<tr>
 		<td> 이름 : ${DetailUser.name }</td>
 		<td> 성별 : ${DetailUser.sex }</td>
 	</tr>
 	<tr>
 		<td> 
 		부서 : <select name = "department" class = "form-control" style = "width:100px;">
                <c:forEach items = "${Detaildepartment }" var = "departmentList">
                <c:if test = "${ DetailUser.department != departmentList.department}">
                   <option value = "${departmentList.department }">${departmentList.department }
					</option>
				</c:if>
				<c:if test = "${ DetailUser.department == departmentList.department}">
                   <option selected = "${DetailUser.department }" value = "${DetailUser.department }">${departmentList.department }
					</option>
				</c:if>
 		...
 		<td> 직급 : <select name = "position" class = "form-control" style = "width:100px;">
 	...
 	<tr >
 		<td colspan = "2"> 주민번호 : ${DetailUser.registernum }</td>
 	</tr>
 	<tr>
 		<td colspan = "2"> 주소 : ${DetailUser.address }</td>
 	</tr>
 	<tr>
 		<td colspan = "2"> 폰번호 : ${DetailUser.phonenum }</td>
 	</tr>
 	<tr>
 		<td colspan = "2"> email : ${DetailUser.email }</td>
 	</tr>
 </table>

	<input type="submit" name = "AdminDelete" class="btn btn-danger" style = "float : right;"value="삭제하기">
	<input type = "submit" name = "AdminUpdate" class = "btn btn-primary" style = "float : right;"value="수정하기">
	<input type = "hidden" name = "nowDepartment" value = "${DetailUser.department }">
</form>
