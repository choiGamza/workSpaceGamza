...
<style>                           
	...
	float: right;
      ...
      <table class="table" style="text-align: center; border: 2px;">
							<caption>
							<c:if test = "${noticedto.name == 'admin' }">
							<span class="badge badge-pill badge-primary">공지</span>
							<span class = "sma">....공지사항 </span>   
						    <span class = "hope">${noticedto.rdate }</span>
							</c:if>
							${noticedto.title } <span class = "textsmall">${noticedto.name }</span>
							<c:if test = "${noticedto.name != 'admin'  }">
							<span class = "sma">....게시글 </span>   
						    <span class = "hope">${noticedto.rdate }</span>
						    </c:if>
							</caption>
							<tr>
							...
							<c:forEach items = "${noticeReplyList }" var = "noticeReplyList">
							<tr class = "table table-striped">
									<th class = "replyid">${noticeReplyList.replyid }</th>
									<td class = "replycontent"> ${noticeReplyList.replycontent }</td>
									
									<td style = "text-align : right; font-size : 12px;">		
									<c:if test = "${user.id == noticeReplyList.replyid}">					
										<a href = "deleteReply?replycontent=${noticeReplyList.replycontent }&&bid=${noticedto.bid}">x</a>
									</c:if>
									<c:if test = "${user.id == 'admin'}">					
										<a href = "deleteReply?replycontent=${noticeReplyList.replycontent }&&bid=${noticedto.bid}">x</a>
									</c:if>
							...
							<tr class = "table table-striped">
								 <td colspan = "3">
								 	<form action = "noticeReplyAdd" method = "post">
								 	<textarea class="form-control" cols="20" rows="3" name = "replycontent"></textarea>
								 	<input type = "hidden" name = "replyid" value = "${user.id }">
								 	<input type = "hidden" name = "bid" value = "${noticedto.bid }">
						...
						<c:if test="${user.id=='admin'}">
							...
							<input type = "submit" class = "btn btn-danger btn-sm" style = "float : right;" value = "삭제">
							<input type = "hidden" name = "bid" value = "${noticedto.bid }">
							</form>
						...
						-->
						<c:if test = "${user.id == noticedto.name or user.id == 'admin'}">
							<form action = "deleteNotice">
								<input type = "submit" class = "btn btn-danger btn-sm" style = "float : right;" value = "삭제">
								<input type = "hidden" name = "bid" value = "${noticedto.bid }">
								</form>
					...
					<div class="modal fade" id="myModal" role="dialog">
						<div class="modal-dialog">
							<div class="modal-content">
								...
								<c:if test = "${noticedto.name == 'admin' }">
									<h3 class="modal-title">공지사항수정</h3>
								</c:if>
								<c:if test = "${noticedto.name != 'admin' }">
									<h3 class="modal-title">게시글수정</h3>
								</c:if>
								...
								<form action="noticeUpdate" class="form-inline" method="post">
									<div class="modal-body">


										제목 <input type="text" value = "${noticedto.title }"
											name="title" class="form-control" style="width: 380px;">
										<br> 내용
										<textarea class="form-control" cols="50" rows="13" name = "content"
											>${noticedto.content }</textarea>
