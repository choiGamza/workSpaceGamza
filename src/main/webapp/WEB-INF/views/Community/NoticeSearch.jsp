      ...
      <table class="table" style="text-align: center; border: 2px;">
							...
							<c:forEach items = "${searchList }" var = "searchList">
								<tr>
									...
									<td>${searchList.bid }</td>
									<td><a href = "noticeView?bid=${searchList.bid }">${searchList.title }</a></td>
									<td>${searchList.name }</td>
									<td>${searchList.rdate }</td>
						...
						<c:forEach items = "${searchPageList }" var = "list" varStatus = "status1">
						...
						<nav aria-label="Page navigation example">
						  <ul class="pagination justify-content-center">
							<c:forEach var = "count" begin = "1" end = "${result+1 }">
								<li class = "page-item">	<a class = "page-link" href = "SearchPage?pagenum=${count }&&Search=${menu}&&Sear=${sear}">${count }</a></li>
							</c:forEach>
						 </ul>
						</nav>
